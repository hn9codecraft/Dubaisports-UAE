...
class ClearanceSaleFilter extends Component
{
    ...
    public function mount() {
        // Fetch only categories that contain clearance sale products
        $categoryIds = Product::where('clearance_sale', '1')
            ->where('status', '1')
            ->pluck('category_id')
            ->unique()
            ->filter();
    ...
    public function removeSelection($id)
    {
        ...
        if ($tempAttributes) {
            $this->products = Product::where('clearance_sale', '1')
                ->where('status', '1')
