                ...
                <div class="col-md-3 border-end pe-md-4">
                    ...
                    <div class="english_preferance mb-md-4 mb-3">
                        <div class="d-flex justify-content-between border-bottom">
                            ...
                            <a data-bs-toggle="modal" href="#englishpopup" role="button"><i
                                    class="fas fa-pencil"></i></a>
                        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="mb-lg-5 mb-md-4 mb-3">
                    ...
                    @endphp
                    {{ Form::select('skills', ['' => 'Select Skills'] + $skills, $data['skills'], ['class' => 'form-select border-0 border-bottom rounded-0 ps-1', 'id' => 'skills']) }}
                    <span class='text-danger skillError'></span>
                </div>
                <div class="mt-4">
...
<div class="modal fade" id="englishpopup" aria-hidden="true" aria-labelledby="englishpopupLabel" tabindex="-1">
    <form id="englishSubmit">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header pb-2">
            ...
            <div class="modal-body">
                <div class="d-flex">
                    <div class="form-check me-3">
                        <input class="form-check-input" type="radio" name="english_fluency" value="Beginner" id="Beginner" @if($data['english_fluency'] == 'Beginner') checked @endif>
                        <label class="form-check-label" for="Beginner">
                            Beginner
                        </label>
                    </div>
                    <div class="form-check me-3">
                        <input class="form-check-input" type="radio" name="english_fluency" value="Intermediate" id="Intermediate" @if($data['english_fluency'] == 'Intermediate') checked @endif>
                        <label class="form-check-label" for="Intermediate">
                            Intermediate
                        </label>
                    </div>
                    <div class="form-check me-3">
                        <input class="form-check-input" type="radio" name="english_fluency" value="Fluent" id="Fluent" @if($data['english_fluency'] == 'Fluent') checked @endif>
                        <label class="form-check-label" for="Fluent">
                            Fluent
                        </label>
                    </div>
                </div>
                <span class='text-danger fluencyError'></span>
                <div class="mt-4">
                    <button class="btn btn-primary text-white">Submit</button>
                </div>
            </div>
...
<!-- Availibility Preferance popup model -->
<div class="modal fade" id="availibilitypopup" aria-hidden="true" aria-labelledby="availibilitypopupLabel"
        tabindex="-1">
    <form id="availibilityForm">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header pb-2">
                <h4 class="modal-title" id="exampleModalToggleLabel">Availibility</h4>
                <button type="button" class="btn-close py-0" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="mb-lg-5 mb-md-4 mb-3">
                    <select name="availability" id="availability" class="form-select border-0 border-bottom rounded-0 ps-1" aria-label="Default select example">
                        <option selected>Availability each day</option>
                        <option value="1 to 3 Hours" @if($data['availability'] == '1 to 3 Hours') selected @endif>1 to 3 Hours</option>
                        <option value="3 to 6 Hours" @if($data['availability'] == '3 to 6 Hours') selected @endif>3 to 6 Hours</option>
                        <option value="More Than 6 Hours" @if($data['availability'] == 'More Than 6 Hours') selected @endif>More Than 6 Hours</option>
                    </select>
                </div>
...
@section('js')
<script>
    $( '#skillSubmit' ).on( 'submit', function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            if(resp.type == 'error') {
                $('.skillError').html(error.responseJSON.message);
            } else {
                $("#skillpopup").modal('hide');
                toastr.success(resp.type, resp.message);
            }
        }).fail(function (error) {
            $('.skillError').html(error.responseJSON.message);
        });
    ...
    $('#availibilityForm' ).on( 'submit', function(e) {
        e.preventDefault();
        var english_fluency = $('input[name="english_fluency"]:checked').val();;
        var _token = '{{ csrf_token() }}';
        $.ajax({
            type: "PATCH",
            url: 'users/' + '{{ auth()->user()->id }}',
            data: { english_fluency, _token  },
        }).done(function( resp ) {
            if(resp.type == 'error') {
                $('.fluencyError').html(error.responseJSON.message);
            } else {
                $("#englishpopup").modal('hide');
                toastr.success(resp.type, resp.message);
            }
        }).fail(function (error) {
            $('.fluencyError').html(error.responseJSON.message);
    ...
    $('#availibilityForm' ).on( 'submit', function(e) {
        ...
        $.ajax({
            type: "PATCH",
            url: 'users/' + '{{ auth()->user()->id }}',
            data: { availability, _token  },
        }).done(function( resp ) {
            if(resp.type == 'error') {
                $('.availabilityError').html(error.responseJSON.message);
            } else {
                $("#availibilitypopup").modal('hide');
                toastr.success(resp.type, resp.message);
            }
        }).fail(function (error) {
            $('.availabilityError').html(error.responseJSON.message);
